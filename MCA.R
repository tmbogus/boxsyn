...
input_file <- "chromos_screen_hittbl.clean.tsv"
output_dir <- "output/"
...
mca_results <- MCA(chromosomes, graph = FALSE)

# Create data frames for plotting
mca_variables_df <- data.frame(mca_results$var$coord, Variable = rep(names(category_counts), category_counts))
mca_observations_df <- data.frame(mca_results$ind$coord)

# Write MCA results to output files
write.table(mca_variables_df, file = paste0(output_dir, "mca4_vars.txt"), sep = "\t", row.names = TRUE, col.names = TRUE)
write.table(mca_observations_df, file = paste0(output_dir, "mca4_obs.txt"), sep = "\t", row.names = TRUE, col.names = TRUE)
...
# Plot ellipses
pdf(paste0(output_dir, "mca4_ellipses.pdf"))
